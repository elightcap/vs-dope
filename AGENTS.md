...
- Animation keyframe offsets, rotations and stretches must specify all three XYZ axes whenever any axis in that group is present. In 1.22.7, `Animation.lerpKeyFrameElement` dereferences all three nullable values; a partial vector crashes on first playback. Validate custom animations with `Shape.InitForAnimations` followed by `Animation.GenerateAllFrames`, not just JSON parsing (see `tests/CannabisProbe/SmokingAnimationChecks.cs`).
- Currency is `game:gear-rusty`.
- Seeds use vanilla's `game:item/resource/seeds/seedbag` shape. The `seed` texture key is a 16x16 plant picture drawn on the bag's label (ours: `textures/item/seeds/{type}.png`, generated by `tools/build_seed_icons.py`). Map `normal`/`reedrope` explicitly to `game:` textures. A bare `"texture"` on an item with no shape renders as a flat voxel of that PNG.
- Renaming a code: ship `assets/vs-dope/config/remaps.json` (`{"<set>": ["/iir remapq <new> <old> force", "/bir ..."]}`). The server merges non-game remaps and auto-applies unseen sets on world load (`ServerSystemRemapperAssistant`). This was verified with the marijuana→cannabis rename.
