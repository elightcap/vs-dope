...
- Held transforms anchor neck point `(6.5, 7, 8)/16` at the native hand attachment: origin is that grip, scale is 0.65 and translation is `-origin/scale`. The held rotation turns the bowl/downstem outward around that neck axis, preserving grip and mouthpiece alignment. GUI X rotation is 165 degrees because item icons do not receive the automatic block flip. Ground rotation explicitly overrides the item default of 90 degrees.
