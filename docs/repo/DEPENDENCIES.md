...
| Marijuana tolerance / hunger / detection | `StonedSystem.cs`, `AddictionSystem.RecordToleranceUse` | Stoned HUD, joint tooltip, MarijuanaProbe, ToolEffectChecks |
...
- Smoking pose/keyframes: `patches/marijuana-player.json` and `patches/bong-player.json` (both Seraph shapes, both camera variants) and `tests/MarijuanaProbe/SmokingAnimationChecks.cs`. Supply complete XYZ transform groups and run native frame generation to catch first-playback failures.
- Stoned keys/timing/strength: `Systems/StonedSystem.cs`, `Client/StonedHudSystem.cs`, `Client/StonedEyesBehavior.cs`, localization. Marijuana uses `AddictionSystem`'s per-product tolerance recording without `RecordDrugDose`; it must not enter the shared overdose window.
