...
| Practical drug effects / coca crash | `DrugToolEffects.cs`, `DrugConsumableItem.cs` | Coca HUD, lang tooltips, `ToolEffectChecks`, `docs/DRUG_TOOLS.md` |
| Cannabis tolerance / hunger / detection | `StonedSystem.cs`, `AddictionSystem.RecordToleranceUse` | Stoned HUD, joint tooltip, CannabisProbe, ToolEffectChecks |
...
`AddictReputationSystem` (ledger owner) <- `EntityDrugAddict` (identity, deaths, muggings, assaults, rumours, release on despawn), `AddictTradeSystem` (tier prices, `RecordSale`, decline-adjusted OD chance), `DrugAddictSpawnSystem` (`PickForSpawn`, `PickRegularVisits`, `SpawnMultiplier`, `BindIdentity` + `MarkActive` after `SpawnEntity`). Exposure weights are keyed by the drug codes in `AddictTradeSystem.Offers`: renaming a drug means updating `AddictReputationSystem.ExposurePerUnit`. Decline stages tie `AddictLedger.DeclineThresholds` (3) to the 3 `alternates` in `entities/drugaddict.json` and `DECLINE_STAGES` in `tools/build_addict_texture.py`. Rumour kinds match vanilla structure codes in `worldgen/structures.json` (`surrfaceruins` is the vanilla spelling). Names are `addict-name-0..` lang keys, `AddictLedger.NameCount` of them. The save-data key `vs-dope-addict-ledger` and every `ProtoMember` number are save format. Issue #52 (runners) should build on `AddictRecord`/`AddictRelation`.

## Cannabis change map

- Stage/count/growth/drop changes: `blocktypes/cannabis-plant.json`, `itemtypes/cannabis-seeds.json`, `tools/build_cannabis_models.py`, language, previews.
- Bud/joint meshes: `shapes/item/{cannabis-buds,joint}.json`, matching itemtypes and shared cannabis atlas.
- Reusable bong: `Items/BongItem.cs` -> class registration -> `itemtypes/bong.json` empty/loaded variants; exact quartz and shapeless bud recipes -> native item models/glass atlas/bubbling audio and `patches/bong-player.json` arm/wrist poses. Server-only completion emits native smoke particles after returning the vessel. Same-slot empty return depends on both variants staying non-stackable. Update `tests/CannabisProbe/BongChecks.cs` and `BongPresentationChecks.cs`, `docs/BONG.md` and generators together.
- Smoking hold time: `JointItem.SmokeSeconds`, player animation patch frame duration, synthesized audio duration/trigger, `tests/CannabisProbe`, `docs/CANNABIS.md`. Current user requirement: **5 seconds**.
- Smoking pose/keyframes: `patches/cannabis-player.json` and `patches/bong-player.json` (both Seraph shapes, both camera variants) and `tests/CannabisProbe/SmokingAnimationChecks.cs`. Supply complete XYZ transform groups and run native frame generation to catch first-playback failures.
- Stoned keys/timing/strength: `Systems/StonedSystem.cs`, `Client/StonedHudSystem.cs`, `Client/StonedEyesBehavior.cs`, localization. Cannabis uses `AddictionSystem`'s per-product tolerance recording without `RecordDrugDose`; it must not enter the shared overdose window.
- Seed access: separate `patches/cannabis-seed-traders.json`, using the same vanilla lists as coca seeds.
