...
- The same script writes `drugaddict-decline1..3.png`, the visible-decline stages (issue #51): each paints on top of the finished base with its own RNG, so the base texture is unchanged. They are the `seraph` texture's `alternates` in `entities/drugaddict.json`; the entity's `textureIndex` watched attribute selects one (0 = base). Stage count must match `DECLINE_STAGES` in the script and `AddictLedger.DeclineThresholds`. Preview of all four: `docs/previews/drugaddict-decline.png`.
...
`blocktypes/cannabis-plant.json` maps nine `shapes/plant/cannabis_stage_*.json` stages. `itemtypes/cannabis-seeds.json` plants stage 1 (hand-maintained, not generated); only stage 9 drops `cannabis-buds`. `itemtypes/cannabis-buds.json` and `joint.json` map 3D item shapes for GUI/hand/ground. Shared 64x64 `textures/block/cannabis/cannabis-atlas.png` uses leaf/bud/paper/stem UV quadrants. Deterministic shapes: `tools/build_cannabis_models.py`; previews: `tools/preview_cannabis_models.py`.

`patches/cannabis-player.json` adds 150-frame smoking animations and client eye behavior; `sounds/player/joint-drag.ogg` provides synthesized drag foley. `textures/entity/stoned-eyes.png` is the exact sclera overlay. `patches/cannabis-seed-traders.json` supplies seeds via existing coca-seed traders. See `docs/CANNABIS.md` for UVs, provenance and client acceptance.

Smoking keyframes require complete XYZ offset vectors, including zero X/Z for the lower arm and neutral offsets at frame 0. Both first- and third-person variants are patched into `seraph-faceless` and `seraph`. `tests/CannabisProbe/SmokingAnimationChecks.cs` resolves these actual shapes and runs the native 1.22.7 frame generator; parsing the patches alone will not detect the nullable-vector crash.

`itemtypes/bong.json` adds empty/loaded variants with distinct `shapes/item/bong-{empty,loaded}.json` meshes. Both use 64x64 RGBA `textures/item/bong-glass.png`; the loaded bowl also uses the cannabis atlas. `recipes/grid/bong-empty.json` requires clear quartz in centre and bottom-centre; `bong-loaded.json` combines one empty bong and one bud. `patches/bong-player.json` supplies dedicated 150-frame arm/wrist use animations for both Seraph models and camera modes. Explicit inventory, ground and grip-centred held transforms (bowl/downstem outward in idle and smoking poses) are checked through native matrices in `BongPresentationChecks`. `sounds/player/bong-bubbles.ogg` provides original bubbling audio. Completion smoke uses native particles without a texture asset. Generators/previews: `tools/build_bong_models.py`, `tools/build_bong_audio.py`, `tools/preview_bong_models.py`. Contracts and client acceptance: `docs/BONG.md`.

## Seed items

`seeds.json` (poppy), `coca-seeds.json` and `cannabis-seeds.json` use the vanilla `game:item/resource/seeds/seedbag` shape, `GroundStorable` behavior and vanilla transforms. The label texture `textures/item/seeds/{type}.png` comes from `tools/build_seed_icons.py`. `config/remaps.json` maps the pre-rename `marijuana` item/block codes to `cannabis` for existing worlds. Tolerance is tracked under the new product id `cannabis`, so old `vs-dope-tolerance-uses-marijuana` counters are not carried over.
