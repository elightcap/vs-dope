...
`itemtypes/bong.json` adds empty/loaded variants with distinct `shapes/item/bong-{empty,loaded}.json` meshes. Both use 64x64 RGBA `textures/item/bong-glass.png`; the loaded bowl also uses the marijuana atlas. `recipes/grid/bong-empty.json` requires clear quartz in centre and bottom-centre; `bong-loaded.json` combines one empty bong and one bud. `sounds/player/bong-bubbles.ogg` provides original bubbling audio. Generators/previews: `tools/build_bong_models.py`, `tools/build_bong_audio.py`, `tools/preview_bong_models.py`. Contracts and client acceptance: `docs/BONG.md`.
